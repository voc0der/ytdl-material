...
python_target_has_curl_cffi() {
    python3 - "$1" <<'PY'
import importlib.util
import sys

sys.path.insert(0, sys.argv[1])
sys.exit(0 if importlib.util.find_spec("curl_cffi") is not None else 1)
PY
}
...
install_ytdlp_impersonation_dependencies() {
    ...
    # curl_cffi is the only thing that has to go here. The downloaded yt-dlp binary runs on
    # this same interpreter, so it picks curl_cffi up off PYTHONPATH. Installing yt-dlp here
    # as well would shadow the binary whose version the UI reports, which is why this used to
    # need the update channel plumbed through the entrypoint.
    if python_target_has_curl_cffi "$target_path"; then
    ...
    mkdir -p "$target_path"
    python3 -m pip install --upgrade --target "$target_path" curl_cffi
