...
describe('UnifiedFileCardComponent', () => {
  ...
  it.each(['small', 'medium', 'large'])('should badge the count on a %s playlist thumbnail, opposite the duration', card_size => {
    component.loading = false;
    component.is_playlist = true;
    component.card_size = card_size;
    component.file_obj = {
      name: 'Example playlist',
    ...
    const badges = fixture.debugElement.query(By.css('.img-div')).queryAll(By.css('.item-count, .duration-time'));
    expect(badges.map(badge => badge.nativeElement.className)).toEqual(['item-count', 'duration-time']);
  ...
  it('should badge the count on a playlist with no thumbnail', () => {
    component.loading = false;
    component.is_playlist = true;
    component.file_obj = {name: 'Empty playlist', duration: 0, registered: Date.now(), uids: []} as any;
    fixture.detectChanges();

    const fallback = fixture.debugElement.query(By.css('.thumbnail-fallback'));
    expect(fallback.query(By.css('mat-icon')).nativeElement.textContent.trim()).toBe('playlist_play');
    expect(fallback.query(By.css('.item-count')).nativeElement.textContent.trim()).toBe('0 items');
  });
  ...
  it('should render a file card title without a count', () => {
    ...
    expect(fixture.debugElement.query(By.css('.item-count'))).toBeNull();
  ...
  describe('grid card', () => {
    beforeEach(() => {
      component.displayDateProperty = 'upload_date';
      component.locale = { ngID: 'en-US' } as any;
      vi.spyOn(Date, 'now').mockReturnValue(Date.UTC(2026, 8, 18, 12));
    });

    afterEach(() => vi.restoreAllMocks());

    it('shows the media icon and relative age below the title', () => {
    ...
    it('keeps the menu functional without navigating to the file', () => {
      setUpFileCard({uid: 'f1', title: 'A song', isAudio: true, duration: 5, upload_date: '2024-09-18', registered: Date.now()});
      const navigate = vi.fn();
      component.goToFile.subscribe(navigate);
    ...
    it('gives a playlist the same layout, with its icon and the date it was made', () => {
      component.is_playlist = true;
      component.loading = false;
      component.file_obj = {name: 'Playlist', uids: ['f1'], duration: 5, registered: Date.UTC(2026, 8, 4, 12)};
      fixture.detectChanges();

      expect(fixture.debugElement.query(By.css('.file-grid-layout'))).not.toBeNull();
      expect(fixture.debugElement.query(By.css('.download-time'))).toBeNull();
      expect(fixture.debugElement.query(By.css('.file-details')).nativeElement.textContent).toContain('Playlist');
      expect(fixture.debugElement.query(By.css('.file-metadata mat-icon')).nativeElement.textContent.trim()).toBe('playlist_play');
      expect(fixture.debugElement.query(By.css('.file-metadata span')).nativeElement.textContent.trim()).toBe('9/4/26');
    });

    it('says an automatic playlist is auto-generated rather than giving it a date', () => {
      component.is_playlist = true;
      component.loading = false;
      component.file_obj = {name: 'Music', auto: true, file_count: 12, duration: 5};
      fixture.detectChanges();

      expect(fixture.debugElement.query(By.css('.file-metadata span')).nativeElement.textContent.trim()).toBe('Auto-generated');
      expect(fixture.debugElement.query(By.css('.item-count')).nativeElement.textContent.trim()).toBe('12 items');
    });

    it('leaves the list layout unchanged', () => {
