...
Generate an API token from **Your Profile** to let scripts and other apps act as you. RSS feeds use a separate feed-only token, sent in the `X-Api-Token` header, generated by the RSS dialog. Enable API documentation in Settings -> *Extra* and restart for endpoint details.
