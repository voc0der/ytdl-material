...
Generate an API token from **Your Profile** to let scripts and other apps act as you. RSS feed URLs use a separate feed-only token generated by the RSS dialog. Enable the public API in Settings -> *Extra*, then enable API docs (restart required) for endpoint details.
