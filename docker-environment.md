...
* `ytdl_redis_connection_string`: optional Redis connection string (`redis://` or `rediss://`) for shared rate-limiting state; when omitted, in-memory rate limiting remains the default
...
If `ytdl_redis_connection_string` is configured, ytdl-material will attempt to use Redis for shared Express rate-limiter state during startup. If Redis is unreachable or the connection string is invalid, the backend logs a warning and continues with the default in-memory limiter store instead.
