// Drives the Archive dialog and the confirmation it opens, end to end.
//
// The archive is the one screen that is only reachable as a dialog, and the only way to see
// whether removing, importing and exporting still work is to do them: they are three
// different endpoints and the dialog is what ties them together. So this seeds an archive,
...
// One archive item per title, as addToArchive writes them, at fixed timestamps a day apart
...
async function openArchive(page) {
    ...
    const dialog = page.getByRole('dialog').filter({ hasText: 'Archive' });
    await dialog.waitFor({ timeout: 15_000 });
    return dialog;
}

// The menu the archive is opened from. Its items are one line high and clip whatever does not
// fit, so a label too long for the panel is a label cut in half rather than one that wraps.
async function theMenu(page) {
    say('Opening the toolbar menu');
    await page.goto(`${BASE}/#/home`, { waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('networkidle').catch(() => {});
    await page.getByRole('button', { name: 'More options' }).click();
    const menu = page.locator('.mat-mdc-menu-panel').last();
    await menu.waitFor({ timeout: 10_000 });
    await shoot(page, 'toolbar-menu-desktop');

    const clipped = await menu.evaluate(panel => [...panel.querySelectorAll('.mat-mdc-menu-item')]
        .filter(item => item.scrollHeight > item.clientHeight + 1)
        .map(item => item.innerText.trim().replace(/\s+/g, ' ')));
    check('every item in the menu fits on its line', clipped.length === 0, clipped.join(', ') || 'none clipped');
    check('the archive is one of them', (await menu.innerText()).includes('Archive'));
...
async function theList(page, seeded) {
    say('Opening the archive from the toolbar menu');
    const dialog = await openArchive(page);
    check('the menu item opens the archive', await dialog.isVisible());
    ...
    check('the count is of the whole archive', (await dialog.locator('.archive-count').innerText()).includes(`${seeded.length} items`),
        (await dialog.locator('.archive-count').innerText()).trim());
    ...
    await shoot(page, 'archive-list-desktop');

    const range = await dialog.locator('.pager-range').innerText();
    check('the pager counts the whole archive', range.includes(`of ${seeded.length}`), range.trim());
...
async function filteringAndSorting(page, dialog, seeded) {
    ...
    const rows = dialog.locator('.archive-row');
    const search = dialog.getByPlaceholder('Search the archive');
    ...
    // The type filter is the server's, not the list's: it asks for a narrower archive.
    await dialog.getByRole('button', { name: /^Type/ }).click();
    await page.getByRole('menuitemradio', { name: 'Audio', exact: true }).click();
    await waitFor('the audio-only archive', async () => (await rows.count()) === AUDIO_COUNT);
...
async function removingThroughTheConfirmation(page, dialog, seeded) {
    ...
    await rows.first().locator('.archive-row-button').click();
    check('one selected row says so', (await dialog.locator('.archive-count').innerText()).includes('1 selected'));
    check('and can be removed', !(await remove.isDisabled()));
    await shoot(page, 'archive-selected-desktop');

    await remove.click();
    const confirm = page.getByRole('dialog').filter({ hasText: 'Remove from archive' });
    ...
    await shoot(page, 'archive-confirm-desktop');

    await confirm.getByRole('button', { name: 'Cancel' }).click();
    await confirm.waitFor({ state: 'detached' }).catch(() => {});
    await sleep(500);
    check('cancelling removes nothing', (await storedArchive()).length === seeded.length, `${(await storedArchive()).length} left`);

    await dialog.getByRole('button', { name: 'Select all' }).click();
    check('Select all selects the whole archive, not just the page',
        (await dialog.locator('.archive-count').innerText()).includes(`${seeded.length} selected`),
        (await dialog.locator('.archive-count').innerText()).trim());

    await dialog.getByRole('button', { name: 'Remove' }).click();
    const confirmAll = page.getByRole('dialog').filter({ hasText: 'Remove from archive' });
    await confirmAll.waitFor({ timeout: 10_000 });
    await confirmAll.getByRole('button', { name: 'Remove' }).click();

    const emptied = await waitFor('the archive to be emptied', async () => (await storedArchive()).length === 0);
    check('confirming removes every selected item from the database', !!emptied);
    await dialog.getByText('Your archive is empty').waitFor({ timeout: 15_000 });
    check('an empty archive says so', true);
    await shoot(page, 'archive-empty-desktop');
}
...
async function importingAndExporting(page, dialog) {
    ...
    await shoot(page, 'archive-import-desktop');
    ...
    await dialog.locator('.archive-row').first().waitFor({ timeout: 15_000 });
    check('the list shows them without reopening the dialog', await dialog.locator('.archive-row').count() === IMPORT_LINES.length);
...
async function phoneAndLight(browser, errors) {
    ...
    const phoneDialog = await openArchive(phone);
    await phoneDialog.locator('.archive-row').first().waitFor({ timeout: 15_000 });
    await shoot(phone, 'archive-list-phone');
    ...
    await shoot(phone, 'archive-sort-sheet-phone');
    await phone.keyboard.press('Escape');
    await phoneDialog.locator('.archive-row').first().locator('.archive-row-button').click();
    await phoneDialog.getByRole('button', { name: 'Remove' }).click();
    await phone.getByRole('dialog').filter({ hasText: 'Remove from archive' }).waitFor({ timeout: 10_000 });
    await shoot(phone, 'archive-confirm-phone');
    await phone.context().close();

    const light = await newPage(browser, 'desktop', errors, 'default');
    const lightDialog = await openArchive(light);
    await lightDialog.locator('.archive-row').first().waitFor({ timeout: 15_000 });
    await shoot(light, 'archive-list-light');
    await lightDialog.locator('.archive-row').first().locator('.archive-row-button').click();
    await lightDialog.getByRole('button', { name: 'Remove' }).click();
    await light.getByRole('dialog').filter({ hasText: 'Remove from archive' }).waitFor({ timeout: 10_000 });
    await shoot(light, 'archive-confirm-light');
...
async function main() {
    ...
    say(`Staging the backend and the archive in ${RUN_DIR}`);
    ...
    const errors = [];
    try {
        check('the seeded archive is what the backend serves', (await storedArchive()).length === seeded.length);
        ...
        // Before the removal, so the other widths are captured with an archive in them.
